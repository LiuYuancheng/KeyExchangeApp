...
int pke_dec(const unsigned char *sk, const unsigned char *c,unsigned long long clen, unsigned char *m, unsigned long long *mlen)
{
	...
	int temp1,temp2;
	int center_point=LAC_Q/2;
	int d2_bound=LAC_Q/2;
	//compute mlen
	*mlen=c2_len/16-ECC_LEN;
	...
	{
		//D2 decoding:compute m*q/2+e1 + m*q/2+e2 in [0,2*LAC_Q]
		temp1=(c2[i]-out[i]+LAC_Q)%LAC_Q;
		temp2=(c2[i+vec_bound]-out[i+vec_bound]+LAC_Q)%LAC_Q;
		
		//shift
		if(temp1<center_point)
		{
			temp1=center_point-temp1+center_point;//mirror around LAC_Q/2
		}
		if(temp2<center_point)
		{
			temp2=center_point-temp2+center_point;//mirror around LAC_Q/2
		}
		//merge erors
		temp1+=temp2-LAC_Q;
	...
	*mlen=c2_len/8-ECC_LEN;
	...
	for(i=0;i<c2_len;i++)
	{
		//compute m*q/2+e in [0,LAC_Q]
		temp=(c2[i]-out[i]+LAC_Q)%LAC_Q;
