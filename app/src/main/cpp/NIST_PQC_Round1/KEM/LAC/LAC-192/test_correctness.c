...
int test_pke_correctness()
{
	...
	for(j=0;j<loop;j++)
	{
		...
		for(i=0;i<CTESTS;i++)
		{
			...
			//printf("i=%d\n",i);
			if(memcmp(k1,k2,LAC_CRYPTO_BYTES)!=0)
			{
				error_num++;
				error_bit=error_bit_num(k1,k2,LAC_CRYPTO_BYTES);
				sum+=error_bit;
				if(error_bit>0)
				{
					...
					printf("\n");
					print_error_bit(k1,k2,LAC_CRYPTO_BYTES);
				}
			}
		}
    ...
    sum_bits=CTESTS*loop*LAC_CRYPTO_BYTES*8;
...
int test_ake_correctness()
{
	...
	for(j=0;j<loop;j++)
	{
		for(i=0;i<CTESTS;i++)
		{
			...
			if(memcmp(k_b,k_a,LAC_CRYPTO_BYTES)!=0)
			{
				error_num++;
			}
		}
