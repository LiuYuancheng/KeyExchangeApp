...
{
	unsigned char seed[SEED_LEN],buf[LAC_CRYPTO_SECRETKEYBYTES+SEED_LEN];
	//check pointer
	...
	memcpy(buf+SEED_LEN,sk_a,LAC_CRYPTO_SECRETKEYBYTES);
	gen_seed(buf,LAC_CRYPTO_SECRETKEYBYTES+SEED_LEN,seed);
...
int crypto_ake_bob_receive(unsigned char *pk_b, unsigned char *sk_b, unsigned char *pk_a, unsigned char *pk, unsigned char *c_in,unsigned char *c_out , unsigned char *k)
{
	...
	unsigned char buf[LAC_CRYPTO_SECRETKEYBYTES+SEED_LEN];
	...
	random_bytes(buf,SEED_LEN);
	memcpy(buf+SEED_LEN,sk_b,LAC_CRYPTO_SECRETKEYBYTES);
	gen_seed(buf,LAC_CRYPTO_SECRETKEYBYTES+SEED_LEN,seed);
