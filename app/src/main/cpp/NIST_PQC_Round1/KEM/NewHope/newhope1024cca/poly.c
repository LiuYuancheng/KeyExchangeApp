...
/*************************************************
* Name:        NewHope_poly_decompress
* 
* Description: De-serialization and subsequent decompression of a polynomial; 
*              approximate inverse of NewHope_poly_compress
*
* Arguments:   - poly *r:                pointer to output polynomial
*              - const unsigned char *a: pointer to input byte array
**************************************************/
...
void NewHope_poly_uniform(poly *a, const unsigned char *seed)
{
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* generate a in blocks of 64 coefficients */
  {
    ...
    NewHope_shake128_absorb(state, extseed, (unsigned int)(NEWHOPE_SYMBYTES+1));
    while(ctr < 64) /* Very unlikely to run more than once */
    {
      NewHope_shake128_squeezeblocks(buf,1,state);
...
{
#if NEWHOPE_K != 8
#error "NewHope_poly_sample in poly.c only supports k=8"
#endif
...
//  uint32_t t, d, a, b, c;
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* Generate noise in blocks of 64 coefficients */
  {
    ...
    NewHope_shake256(buf,128,extseed,NEWHOPE_SYMBYTES+2);
    for(j=0;j<64;j++)
    {
      a = buf[2*j];
      b = buf[2*j+1];
