...
/*************************************************
* Name:        NewHope_crypto_kem_keypair
*
...
int NewHope_crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  NewHope_shake256(sk, NEWHOPE_SYMBYTES, pk, NEWHOPE_CPAPKE_PUBLICKEYBYTES);        /* Append the hash of the public key */
  sk += NEWHOPE_SYMBYTES;
...
int NewHope_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{
  ...
  NewHope_shake256(buf,NEWHOPE_SYMBYTES,buf,NEWHOPE_SYMBYTES);                                        /* Don't release system RNG output */
  NewHope_shake256(buf+NEWHOPE_SYMBYTES, NEWHOPE_SYMBYTES, pk, NEWHOPE_CCAKEM_PUBLICKEYBYTES);        /* Multitarget countermeasure for coins + contributory KEM */
  NewHope_shake256(k_coins_d, 3*NEWHOPE_SYMBYTES, buf, 2*NEWHOPE_SYMBYTES);
  ...
  NewHope_shake256(k_coins_d+NEWHOPE_SYMBYTES, NEWHOPE_SYMBYTES, ct, NEWHOPE_CCAKEM_CIPHERTEXTBYTES); /* overwrite coins in k_coins_d with h(c) */
  NewHope_shake256(ss, NEWHOPE_SYMBYTES, k_coins_d, 2*NEWHOPE_SYMBYTES);                              /* hash concatenation of pre-k and h(c) to ss */
  return 0;
}

/*************************************************
* Name:        NewHope_crypto_kem_dec
*
...
int NewHope_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk)
{
  ...
  NewHope_shake256(ss, NEWHOPE_SYMBYTES, k_coins_d, 2*NEWHOPE_SYMBYTES);                              /* hash concatenation of pre-k and h(c) to k */
