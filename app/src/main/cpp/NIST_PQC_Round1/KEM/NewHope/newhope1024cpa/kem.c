...
int NewHope_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{
  ...
  NewHope_shake256(buf,2*NEWHOPE_SYMBYTES,buf,NEWHOPE_SYMBYTES);                         /* Don't release system RNG output */

  cpapke_enc(ct, buf, pk, buf+NEWHOPE_SYMBYTES);                                 /* coins are in buf+NEWHOPE_SYMBYTES */

  NewHope_shake256(ss, NEWHOPE_SYMBYTES, buf, NEWHOPE_SYMBYTES);                         /* hash pre-k to ss */
  return 0;
}
...
int NewHope_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk)
{
  ...
  NewHope_shake256(ss, NEWHOPE_SYMBYTES, ss, NEWHOPE_SYMBYTES);                          /* hash pre-k to ss */
