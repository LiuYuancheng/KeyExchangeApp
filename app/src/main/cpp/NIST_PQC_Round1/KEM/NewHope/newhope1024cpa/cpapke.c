...
{
  int i;
  newhope1024cpa_poly_frombytes(pk, r);
  for(i=0;i<NEWHOPE_SYMBYTES;i++)
...
{
  poly_tobytes(r,b);
  newhope1024cpa_poly_compress(r+NEWHOPE_POLYBYTES,v);
}
...
{
  newhope1024cpa_poly_frombytes(b, r);
  newhope1024cpa_poly_decompress(v, r+NEWHOPE_POLYBYTES);
}
...
{
  newhope1024cpa_poly_uniform(a,seed);
}
                    ...
                    unsigned char *sk)
{
  ...
  newhope1024cpa_poly_sample(&shat, noiseseed, 0);
  newhope1024cpa_poly_ntt(&shat);

  newhope1024cpa_poly_sample(&ehat, noiseseed, 1);
  newhope1024cpa_poly_ntt(&ehat);

  poly_mul_pointwise(&ahat_shat, &shat, &ahat);
  newhope1024cpa_poly_add(&bhat, &ehat, &ahat_shat);
                ...
                const unsigned char *coin)
{
  ...
  newhope1024cpa_poly_frommsg(&v, m);

  decode_pk(&bhat, publicseed, pk);
  gen_a(&ahat, publicseed);

  newhope1024cpa_poly_sample(&sprime, coin, 0);
  newhope1024cpa_poly_sample(&eprime, coin, 1);
  newhope1024cpa_poly_sample(&eprimeprime, coin, 2);

  newhope1024cpa_poly_ntt(&sprime);
  newhope1024cpa_poly_ntt(&eprime);

  poly_mul_pointwise(&uhat, &ahat, &sprime);
  newhope1024cpa_poly_add(&uhat, &uhat, &eprime);

  poly_mul_pointwise(&vprime, &bhat, &sprime);
  newhope1024cpa_poly_invntt(&vprime);

  newhope1024cpa_poly_add(&vprime, &vprime, &eprimeprime);
  newhope1024cpa_poly_add(&vprime, &vprime, &v); // add message
                ...
                const unsigned char *sk)
{
  ...
  newhope1024cpa_poly_frombytes(&shat, sk);

  decode_c(&uhat, &vprime, c);
  poly_mul_pointwise(&tmp, &shat, &uhat);
  newhope1024cpa_poly_invntt(&tmp);

  newhope1024cpa_poly_sub(&tmp, &tmp, &vprime);
