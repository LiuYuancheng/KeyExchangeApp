...
void GenMatrix(polyvec *a, const unsigned char *seed) 
{
  ...
  shake128(buf,(unsigned long long)byte_bank_length,seed,(unsigned long long)SABER_SEEDBYTES);
...
void indcpa_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  shake128(seed, (unsigned long long)SABER_SEEDBYTES, seed, (unsigned long long)SABER_SEEDBYTES); // for not revealing system RNG state
