...
{
  aes256ctr((unsigned char *) L,4*p,aes_nonce,k);
}
...
int crypto_kem_ntrulpr857_ref_keypair(unsigned char *pk,unsigned char *sk)
{
  KEM_KeyGen(pk,sk);
  return 0;
}

int crypto_kem_ntrulpr857_ref_enc(unsigned char *c,unsigned char *k,const unsigned char *pk)
{
  Encap(c,k,pk);
  return 0;
}

int crypto_kem_ntrulpr857_ref_dec(unsigned char *k,const unsigned char *c,const unsigned char *sk)
{
  Decap(k,c,sk);
  return 0;
}
