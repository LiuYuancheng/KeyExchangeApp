...
void KYBER_shake128_absorb(keccak_state *s, const unsigned char *input, unsigned char x, unsigned char y)
{
  ...
  extseed[i++] = x;
  extseed[i]   = y;
  KYBER_shake128_absorb(s->s, extseed, (unsigned long long)(KYBER_SYMBYTES+2));
}
...
{
  KYBER_shake128_squeezeblocks(output, nblocks, s->s);
}
...
**************************************************/
void KYBER_shake256_prf(unsigned char *output, unsigned long long outlen, const unsigned char *key, const unsigned char nonce)
{
  ...
  KYBER_shake256(output, outlen, extkey, KYBER_SYMBYTES+1);
}
