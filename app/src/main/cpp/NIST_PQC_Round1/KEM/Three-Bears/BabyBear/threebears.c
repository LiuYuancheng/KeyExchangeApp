...
void BabyBear_get_pubkey(uint8_t *pk, const uint8_t *seed) {
    hash_ctx_t ctx;
...
void BabyBear_encapsulate(
    ...
    for (unsigned i=0; i<DIM; i++) {
        expand_x(b, &pk[MATRIX_SEED_BYTES+i*GF_BYTES]);
...
#endif
    BabyBear_encapsulate(shared_secret,capsule2,pk,seed);
