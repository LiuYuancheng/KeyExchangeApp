...
int testkem_nts(int iterations)
{
    ...
    do {
        ...
        */
        ...
        pk = (uint8_t *)calloc(NTS_CRYPTO_PUBLICKEYBYTES, sizeof(uint8_t));
        sk = (uint8_t *)calloc(NTS_CRYPTO_SECRETKEYBYTES, sizeof(uint8_t));
        if (NTS_crypto_kem_keypair(pk, sk))
            status = 0;
        
        ciphertext = (uint8_t *)calloc(NTS_CRYPTO_CIPHERTEXTBYTES, sizeof(uint8_t));
        encap_key = (uint8_t *)calloc(NTS_CRYPTO_BYTES, sizeof(uint8_t));
        decap_key = (uint8_t *)calloc(NTS_CRYPTO_BYTES, sizeof(uint8_t));

        if (NTS_crypto_kem_enc(ciphertext, encap_key, pk))
            status = 0;
        
        if (NTS_crypto_kem_dec(decap_key, ciphertext, sk))
            status = 0;
        
        status &= (0 == memcmp(encap_key, decap_key, NTS_CRYPTO_BYTES));
