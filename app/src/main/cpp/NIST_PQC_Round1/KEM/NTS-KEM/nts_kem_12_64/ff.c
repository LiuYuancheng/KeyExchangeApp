...
{   
    ...
    /** 
     *  GF(2^12) is generated by irreducible polynomial x^12 + x^3 + 1,
     *  which is not a primitive polynomial. Let b be a generator of
     *  GF(2^12) whose minimal polynomial is x^12 + x^3 + 1, the primitive
     *  element of GF(2^12) is a = 1 + b + b^2.
     **/
    ff2m->log2poly[0] = 1; ff2m->poly2log[0] = 0xFFFF;
    ff2m->log2poly[1] = 7; ff2m->poly2log[7] = 1;
    for (i=2; i<(1<<ff2m->m)-1; i++) {
        a  =  ff2m->log2poly[i-1];
        a ^= (ff2m->log2poly[i-1] << 1);
        a ^= (ff2m->log2poly[i-1] << 2);
        a ^= (((a & (1 << ff2m->m))     >> ff2m->m)     * 0x1009); /* x^12 + x^3 + 1 */
        a ^= (((a & (1 << (ff2m->m+1))) >> (ff2m->m+1)) * 0x2012); /* x^13 + x^4 + x */
