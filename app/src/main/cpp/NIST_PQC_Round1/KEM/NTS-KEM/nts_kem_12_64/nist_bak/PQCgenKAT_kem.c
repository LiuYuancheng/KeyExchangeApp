...
main()
{
    ...
    unsigned char       ct[NTS_CRYPTO_CIPHERTEXTBYTES], ss[NTS_CRYPTO_BYTES], ss1[NTS_CRYPTO_BYTES];
    int                 count;
    int                 done;
    unsigned char       pk[NTS_CRYPTO_PUBLICKEYBYTES], sk[NTS_CRYPTO_SECRETKEYBYTES];
    int                 i, ret_val;
    
    /* Create the REQUEST file */
    sprintf(fn_req, "PQCkemKAT_%d.req", NTS_CRYPTO_SECRETKEYBYTES);
    if ( (fp_req = fopen(fn_req, "w")) == NULL ) {
        fprintf(stderr, "Couldn't open <%s> for write\n", fn_req);
        return KAT_FILE_OPEN_ERROR;
    }
    sprintf(fn_rsp, "PQCkemKAT_%d.rsp", NTS_CRYPTO_SECRETKEYBYTES);
    ...
    do {
        ...
        if ( (ret_val = NTS_crypto_kem_keypair(pk, sk)) != 0) {
            fprintf(stderr, "crypto_kem_keypair returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        fprintBstr(fp_rsp, "pk = ", pk, NTS_CRYPTO_PUBLICKEYBYTES);
        fprintBstr(fp_rsp, "sk = ", sk, NTS_CRYPTO_SECRETKEYBYTES);
...
#endif
        if ( (ret_val = NTS_crypto_kem_enc(ct, ss, pk)) != 0) {
            fprintf(stderr, "crypto_kem_enc returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        fprintBstr(fp_rsp, "ct = ", ct, NTS_CRYPTO_CIPHERTEXTBYTES);
        fprintBstr(fp_rsp, "ss = ", ss, NTS_CRYPTO_BYTES);
...
#endif
        if ( (ret_val = NTS_crypto_kem_dec(ss1, ct, sk)) != 0) {
            fprintf(stderr, "crypto_kem_dec returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( memcmp(ss, ss1, NTS_CRYPTO_BYTES) ) {
