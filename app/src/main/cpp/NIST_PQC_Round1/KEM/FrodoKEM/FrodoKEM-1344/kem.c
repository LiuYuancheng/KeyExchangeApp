...
{ // FrodoKEM's key generation
  ...
  //          secret key sk (FRODOKEM_CRYPTO_BYTES + BYTES_SEED_A + (PARAMS_LOGQ*PARAMS_N*PARAMS_NBAR)/8 + 2*PARAMS_N*PARAMS_NBAR + BYTES_PKHASH bytes)
    uint8_t *pk_seedA = &pk[0];
    uint8_t *pk_b = &pk[BYTES_SEED_A];
    uint8_t *sk_s = &sk[0];
    uint8_t *sk_pk = &sk[FRODOKEM_CRYPTO_BYTES];
    uint8_t *sk_S = &sk[FRODOKEM_CRYPTO_BYTES + FRODOKEM_CRYPTO_PUBLICKEYBYTES];
    uint8_t *sk_pkh = &sk[FRODOKEM_CRYPTO_BYTES + FRODOKEM_CRYPTO_PUBLICKEYBYTES + 2*PARAMS_N*PARAMS_NBAR];
    ...
    uint8_t randomness[2*FRODOKEM_CRYPTO_BYTES + BYTES_SEED_A];      // contains secret data via randomness_s and randomness_seedSE
    ...
    randombytes(randomness, FRODOKEM_CRYPTO_BYTES + FRODOKEM_CRYPTO_BYTES + BYTES_SEED_A);
    ...
    shake((uint8_t*)S, 2*PARAMS_N*PARAMS_NBAR*sizeof(uint16_t), shake_input_seedSE, 1 + FRODOKEM_CRYPTO_BYTES);
    frodo_sample_n(S, PARAMS_N*PARAMS_NBAR);
    frodo_sample_n(E, PARAMS_N*PARAMS_NBAR);
    frodo_mul_add_as_plus_e(B, S, E, pk);

    // Encode the second part of the public key
    frodo_pack(pk_b, FRODOKEM_CRYPTO_PUBLICKEYBYTES - BYTES_SEED_A, B, PARAMS_N*PARAMS_NBAR, PARAMS_LOGQ);
    ...
    shake(sk_pkh, BYTES_PKHASH, pk, FRODOKEM_CRYPTO_PUBLICKEYBYTES);
...
{ // FrodoKEM's key encapsulation
    ...
    shake(pkh, BYTES_PKHASH, pk, FRODOKEM_CRYPTO_PUBLICKEYBYTES);
    randombytes(mu, BYTES_MU);
    shake(G2out, FRODOKEM_CRYPTO_BYTES + FRODOKEM_CRYPTO_BYTES, G2in, BYTES_PKHASH + BYTES_MU);

    // Generate Sp and Ep, and compute Bp = Sp*A + Ep. Generate A on-the-fly
    shake_input_seedSE[0] = 0x96;
    memcpy(&shake_input_seedSE[1], seedSE, FRODOKEM_CRYPTO_BYTES);
    shake((uint8_t*)Sp, (2*PARAMS_N+PARAMS_NBAR)*PARAMS_NBAR*sizeof(uint16_t), shake_input_seedSE, 1 + FRODOKEM_CRYPTO_BYTES);
    ...
    frodo_unpack(B, PARAMS_N*PARAMS_NBAR, pk_b, FRODOKEM_CRYPTO_PUBLICKEYBYTES - BYTES_SEED_A, PARAMS_LOGQ);
...
{ // FrodoKEM's key decapsulation
    ...
    const uint8_t *sk_pkh = &sk[FRODOKEM_CRYPTO_BYTES + FRODOKEM_CRYPTO_PUBLICKEYBYTES + 2*PARAMS_N*PARAMS_NBAR];
    ...
    shake((uint8_t*)Sp, (2*PARAMS_N+PARAMS_NBAR)*PARAMS_NBAR*sizeof(uint16_t), shake_input_seedSEprime, 1 + FRODOKEM_CRYPTO_BYTES);
    frodo_sample_n(Sp, PARAMS_N*PARAMS_NBAR);
    frodo_sample_n(Ep, PARAMS_N*PARAMS_NBAR);
