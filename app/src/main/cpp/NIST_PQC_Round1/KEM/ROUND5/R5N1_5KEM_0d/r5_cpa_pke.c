...
int r5_cpa_pke_keygen(unsigned char *pk, unsigned char *sk, const parameters *params) {
    ...
    /* Allocate space */
    sigma = (unsigned char *)checked_malloc(params->kappa_bytes);
    A = (uint16_t *)checked_malloc(len_a * sizeof (*A));
    S = (int16_t *)checked_malloc(len_s * sizeof (*S));
    S_T = (int16_t *)checked_malloc(len_s * sizeof (*S_T));
    B = (uint16_t *)checked_malloc(len_b * sizeof (*B));
...
int r5_cpa_pke_encrypt(unsigned char *ct, const unsigned char *pk, const unsigned char *m, const unsigned char *rho, const parameters *params) {
    ...
    sigma = (unsigned char *)checked_malloc(params->kappa_bytes);
    A = (uint16_t *)checked_malloc(len_a * sizeof (*A));
    A_T = (uint16_t *)checked_malloc(len_a * sizeof (*A_T));
    R = (int16_t *)checked_malloc(len_r * sizeof (*R));
    R_T = (int16_t *)checked_malloc(len_r * sizeof (*R_T));
    U = (uint16_t *)checked_malloc(len_u * sizeof (*U));
    U_T = (uint16_t *)checked_malloc(len_u * sizeof (*U_T));
    B = (uint16_t *)checked_malloc(len_b * sizeof (*B));
    B_T = (uint16_t *)checked_malloc(len_b * sizeof (*B_T));
    X = (uint16_t *)checked_malloc(len_x * sizeof (*X));
    v = (uint16_t *)checked_malloc(params->mu * sizeof (*v));
    m1 = (unsigned char *)checked_malloc(len_m1 * sizeof (*m1));
...
int r5_cpa_pke_decrypt(unsigned char *m, const unsigned char *sk, const unsigned char *ct, const parameters *params) {
    ...
    S_T = (int16_t *)checked_malloc(len_s * sizeof (*S_T));
    U = (uint16_t *)checked_malloc(len_u * sizeof (*U));
    U_T = (uint16_t *)checked_malloc(len_u * sizeof (*U));
    v = (uint16_t*)checked_malloc(params->mu * sizeof (*v));
    X_prime = (uint16_t *)checked_malloc(len_x_prime * sizeof (*X_prime));
    m2 = (uint16_t *)checked_malloc(params->mu * sizeof (*m2));

    /* Message plus error correction */
    len_m1 = (size_t) BITS_TO_BYTES(params->mu * params->b_bits);
    m1 = (uint8_t*)checked_calloc(len_m1, 1);
