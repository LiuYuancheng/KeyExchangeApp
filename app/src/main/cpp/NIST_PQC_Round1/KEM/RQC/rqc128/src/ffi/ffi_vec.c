...
/** 
 * \fn void ffi_vec_set_random(ffi_vec& o, unsigned int size, RQC_AES_XOF_struct* ctx)
...
/** 
 * \fn void ffi_vec_set_random_full_rank(ffi_vec& o, unsigned int size, RQC_AES_XOF_struct* ctx)
...
/** 
 * \fn void ffi_vec_set_random_full_rank_with_one(ffi_vec& o, unsigned int size, RQC_AES_XOF_struct* ctx)
...
/** 
 * \fn void ffi_vec_set_random_from_support(ffi_vec& o, unsigned int size, const ffi_vec& support, unsigned int rank, RQC_AES_XOF_struct* ctx)
...
void ffi_vec_to_string(unsigned char* str, const ffi_vec& v, unsigned int size) {
  size = deg(v);
	for(unsigned int i = 0 ; i <= size ; i++) {
		ffi_elt_to_string(str + i * RQC_FIELD_ELT_BYTES, v[i]);
	}
}
...
void ffi_vec_from_string(ffi_vec& v, unsigned int size, const unsigned char *str) {
  ffi_vec_set_length(v, size);
	for(unsigned int i = 0 ; i < size ; i++) {
		ffi_elt_from_string(v[i], str + i * RQC_FIELD_ELT_BYTES);
	}
}
