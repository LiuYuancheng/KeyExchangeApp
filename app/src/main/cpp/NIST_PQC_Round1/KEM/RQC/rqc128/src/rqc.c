...
void rqc_pke_keygen(unsigned char* pk, unsigned char* sk) {
  ...
  ffi_vec_set_random_full_rank_with_one(support_w, RQC_PARAM_W, sk_seedexpander);
  ffi_vec_set_random_from_support(x, RQC_PARAM_N, support_w, RQC_PARAM_W, sk_seedexpander);
  ffi_vec_set_random_from_support(y, RQC_PARAM_N, support_w, RQC_PARAM_W, sk_seedexpander);

  // Compute public key
  ffi_vec g, h, s;
  ffi_vec_set_random_full_rank(g, RQC_PARAM_N, pk_seedexpander);
  ffi_vec_set_random(h, RQC_PARAM_N, pk_seedexpander); 
  
  ffi_vec_mul(s, h, y, RQC_PARAM_N);
  ffi_vec_add(s, s, x, RQC_PARAM_N);
  ...
  rqc_secret_key_to_string(sk, sk_seed, pk);
    ...
    printf("\n\nsupport_w: "); ffi_vec_print(support_w, RQC_PARAM_W);
    printf("\nx: "); ffi_vec_print(x, RQC_PARAM_N);
    printf("\ny: "); ffi_vec_print(y, RQC_PARAM_N);

    printf("\npk_seed: "); for(int i = 0 ; i < SEEDEXPANDER_SEED_BYTES ; ++i) printf("%02x", pk_seed[i]);
    printf("\n\ng: "); ffi_vec_print(g, RQC_PARAM_N);
    printf("\nh: "); ffi_vec_print(h, RQC_PARAM_N);
    printf("\ns: "); ffi_vec_print(s, RQC_PARAM_N);
...
void rqc_pke_encrypt(ffi_vec& u, ffi_vec& v, const ffi_vec& m, unsigned char* theta, const unsigned char* pk) {
  ...
  ffi_vec_set_random_full_rank(support_r, RQC_PARAM_W_R, seedexpander);
  ffi_vec_set_random_from_support(r1, RQC_PARAM_N, support_r, RQC_PARAM_W_R, seedexpander);
  ffi_vec_set_random_from_support(r2, RQC_PARAM_N, support_r, RQC_PARAM_W_R, seedexpander);
  ffi_vec_set_random_from_support(e, RQC_PARAM_N, support_r, RQC_PARAM_W_R, seedexpander);

  // Compute u = r1 + h.r2
  ffi_vec_mul(u, h, r2, RQC_PARAM_N);
  ffi_vec_add(u, u, r1, RQC_PARAM_N);
  ...
  ffi_vec tmp;
  ffi_vec_mul(tmp, s, r2, RQC_PARAM_N);
  ffi_vec_add(tmp, tmp, e, RQC_PARAM_N);
  ffi_vec_add(v, v, tmp, RQC_PARAM_N);

  #ifdef VERBOSE
    printf("\n\nh: "); ffi_vec_print(h, RQC_PARAM_N);
    printf("\ns: "); ffi_vec_print(s, RQC_PARAM_N);
    printf("\nsupport_r: "); ffi_vec_print(support_r, RQC_PARAM_W_R);
    printf("\nr1: "); ffi_vec_print(r1, RQC_PARAM_N);
    printf("\nr2: "); ffi_vec_print(r2, RQC_PARAM_N);
    printf("\ne: "); ffi_vec_print(e, RQC_PARAM_N);

    printf("\nu: "); ffi_vec_print(u, RQC_PARAM_N);
    printf("\nv: "); ffi_vec_print(v, RQC_PARAM_N);
  #endif
...
void rqc_pke_decrypt(ffi_vec& m, const ffi_vec& u, const ffi_vec& v, const unsigned char* sk) {
  ...
  unsigned char pk[RQC_CRYPTO_PUBLICKEYBYTES];
  ffi_vec x, y;
  ffi_vec g, h, s; 
  ...
  // Compute v - u.y
  ffi_vec tmp;
  ffi_vec_mul(tmp, u, y, RQC_PARAM_N);
  ffi_vec_add(tmp, v, tmp, RQC_PARAM_N);

  #ifdef VERBOSE
    printf("\n\nu: "); ffi_vec_print(u, RQC_PARAM_N);
    printf("\nv: "); ffi_vec_print(v, RQC_PARAM_N);
    printf("\ny: "); ffi_vec_print(y, RQC_PARAM_N);
    printf("\nv - u.y: "); ffi_vec_print(tmp, RQC_PARAM_N);
  #endif
