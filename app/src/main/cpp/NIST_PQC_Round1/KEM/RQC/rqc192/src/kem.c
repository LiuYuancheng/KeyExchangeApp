...
int RQC_crypto_kem_enc(unsigned char* ct, unsigned char* ss, const unsigned char* pk) {
  ...
  // Computing m
  ffi_vec m;
  ffi_vec_set_random2(m, RQC_PARAM_K);

  // Generating G function
  unsigned char seed_G[RQC_VEC_K_BYTES];
  ffi_vec_to_string_compact(seed_G, m, RQC_PARAM_K);
  ...
  ffi_vec_to_string(m_str, m, RQC_PARAM_K);
  ...
  ffi_vec_to_string(u_str, u, RQC_PARAM_N);

  unsigned char v_str[RQC_FFI_VEC_N_BYTES];
  ffi_vec_to_string(v_str, v, RQC_PARAM_N);
...
int RQC_crypto_kem_dec(unsigned char* ss, const unsigned char* ct, const unsigned char* sk) {
  ...
  int abort = 0;
  if(ffi_vec_is_equal_to(u, u2, RQC_PARAM_N) == 0 || ffi_vec_is_equal_to(v, v2, RQC_PARAM_N) == 0) {
    abort = 1;
  }
  ...
  #ifdef VERBOSE
    printf("\nu2: "); ffi_vec_print(u2, RQC_PARAM_N);
    printf("\nv2: "); ffi_vec_print(v2, RQC_PARAM_N);
