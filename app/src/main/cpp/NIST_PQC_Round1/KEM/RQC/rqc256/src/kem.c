...
/** 
 * \fn int RQC_crypto_kem_keypair(unsigned char* pk, unsigned char* sk)
...
/** 
 * \fn int RQC_crypto_kem_enc(unsigned char* ct, unsigned char* ss, const unsigned char* pk)
...
int RQC_crypto_kem_enc(unsigned char* ct, unsigned char* ss, const unsigned char* pk) {
  ...
  // Computing m
  RQC_ffi_vec m;
  RQC_ffi_vec_set_random2(m, RQC_PARAM_K);

  // Generating G function
  unsigned char seed_G[RQC_VEC_K_BYTES];
  RQC_ffi_vec_to_string_compact(seed_G, m, RQC_PARAM_K);
  ...
  seedexpander(G_seedexpander, theta, SEEDEXPANDER_SEED_BYTES);
    ...
    printf("\n\nm: "); RQC_ffi_vec_print(m, RQC_PARAM_K);
  ...
  unsigned char m_str[RQC_FFI_VEC_K_BYTES];
  RQC_ffi_vec_to_string(m_str, m, RQC_PARAM_K);
  sha512(d, m_str, RQC_FFI_VEC_K_BYTES);

  // Computing ciphertext
  rqc_kem_ciphertext_to_string(ct, u, v, d);
  
  // Computing shared secret
  unsigned char u_str[RQC_FFI_VEC_N_BYTES];
  RQC_ffi_vec_to_string(u_str, u, RQC_PARAM_N);

  unsigned char v_str[RQC_FFI_VEC_N_BYTES];
  RQC_ffi_vec_to_string(v_str, v, RQC_PARAM_N);

  unsigned char mc[RQC_FFI_VEC_K_BYTES + 2 * RQC_FFI_VEC_N_BYTES];
  memcpy(mc, m_str, RQC_FFI_VEC_K_BYTES);
  memcpy(mc + RQC_FFI_VEC_K_BYTES, u_str, RQC_FFI_VEC_N_BYTES);
  memcpy(mc + RQC_FFI_VEC_K_BYTES + RQC_FFI_VEC_N_BYTES, v_str, RQC_FFI_VEC_N_BYTES);
  sha512(ss, mc, RQC_FFI_VEC_K_BYTES + 2 * RQC_FFI_VEC_N_BYTES);
...
/** 
 * \fn int RQC_crypto_kem_dec(unsigned char* ss, const unsigned char* ct, const unsigned char* sk)
...
int RQC_crypto_kem_dec(unsigned char* ss, const unsigned char* ct, const unsigned char* sk) {
  ...
  int abort = 0;
  if(RQC_ffi_vec_is_equal_to(u, u2, RQC_PARAM_N) == 0 || RQC_ffi_vec_is_equal_to(v, v2, RQC_PARAM_N) == 0) {
    abort = 1;
  }
  ...
  unsigned char m_str[RQC_FFI_VEC_K_BYTES];
  RQC_ffi_vec_to_string(m_str, m, RQC_PARAM_K);
  sha512(d2, m_str, RQC_FFI_VEC_K_BYTES);
  ...
  #ifdef VERBOSE
    printf("\nu2: "); RQC_ffi_vec_print(u2, RQC_PARAM_N);
    printf("\nv2: "); RQC_ffi_vec_print(v2, RQC_PARAM_N);
