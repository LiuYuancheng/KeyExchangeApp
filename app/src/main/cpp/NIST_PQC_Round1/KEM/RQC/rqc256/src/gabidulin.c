...
/** 
 * \fn gabidulin_code gabidulin_code_init(const RQC_ffi_vec& g, unsigned int k, unsigned int n)
...
/** 
 * \fn void gabidulin_code_encode(RQC_ffi_vec& c, gabidulin_code gc, const RQC_ffi_vec& m)
...
void gabidulin_code_encode(RQC_ffi_vec& c, gabidulin_code gc, const RQC_ffi_vec& m) {
  ...
  RQC_ffi_elt matrix[RQC_PARAM_K][RQC_PARAM_N];
  for(unsigned int j = 0 ; j < gc.n ; ++j) {
    RQC_ffi_elt_set(matrix[0][j], gc.g[j]);
    for(unsigned int i = 1 ; i < gc.k ; ++i) {
      RQC_ffi_elt_sqr(matrix[i][j], matrix[i-1][j]);
    }
  }

  // Encode message
  RQC_ffi_elt tmp;
  RQC_ffi_elt_set_zero(tmp);
  RQC_ffi_vec_set_zero(c, gc.n);
  for(unsigned int i = 0 ; i < gc.k ; ++i) {
    for(unsigned int j = 0 ; j < gc.n ; ++j) {
      RQC_ffi_elt_mul(tmp, m[i], matrix[i][j]);
      RQC_ffi_elt_add(tmp, c[j], tmp);
      RQC_ffi_vec_set_coeff(c, tmp, j);
    }
  }

  #ifdef VERBOSE
    printf("\n\n\n# Gabidulin Encoding - Begin #");
    printf("\n\ng: "); RQC_ffi_vec_print(gc.g, RQC_PARAM_N);

    printf("\nmatrix:[ ");
    for(unsigned int i = 0 ; i < gc.k ; ++i) {
      printf("[ ");
      for(unsigned int j = 0 ; j < gc.n ; ++j) {
        RQC_ffi_elt_print(matrix[i][j]);
      }
      printf("] ");
    }
    printf("]\n");

    printf("\ncodeword: "); RQC_ffi_vec_print(c, RQC_PARAM_N);
...
/** 
 * \fn void gabidulin_code_decode(RQC_ffi_vec& m, gabidulin_code gc, const RQC_ffi_vec& y)
...
void gabidulin_code_decode(RQC_ffi_vec& m, gabidulin_code gc, const RQC_ffi_vec& y) {
  ...
  for(unsigned int i = 0 ; i < gc.n ; ++i) {
    q_polynomial_evaluate(tmp1, A, RQC_ffi_vec_get_coeff(gc.g, i));
    q_polynomial_evaluate(tmp2, V0, RQC_ffi_vec_get_coeff(y, i));
    RQC_ffi_elt_add(tmp1, tmp1, tmp2);
    RQC_ffi_vec_set_coeff(u0, tmp1, i);

    q_polynomial_evaluate(tmp1, I, RQC_ffi_vec_get_coeff(gc.g, i));
    q_polynomial_evaluate(tmp2, V1, RQC_ffi_vec_get_coeff(y, i));
    RQC_ffi_elt_add(tmp1, tmp1, tmp2);
    RQC_ffi_vec_set_coeff(u1, tmp1, i);
  }
  ...
  for(unsigned int i = gc.k ; i < gc.n ; ++i) {
    ...
    unsigned int j = i;
    while(RQC_ffi_elt_is_zero(RQC_ffi_vec_get_coeff(u0, j)) == 0 && 
          RQC_ffi_elt_is_zero(RQC_ffi_vec_get_coeff(u1, j)) == 1 &&
          j < gc.n) j++;
    ...
    if(j == gc.n) {
      ...
      if(i != j) {
        ...
        RQC_ffi_elt_set(tmp1, RQC_ffi_vec_get_coeff(u0, i));
        RQC_ffi_vec_set_coeff(u0, RQC_ffi_vec_get_coeff(u0, j), i);
        RQC_ffi_vec_set_coeff(u0, tmp1, j);

        RQC_ffi_elt_set(tmp1, RQC_ffi_vec_get_coeff(u1, i));
        RQC_ffi_vec_set_coeff(u1, RQC_ffi_vec_get_coeff(u1, j), i);
        RQC_ffi_vec_set_coeff(u1, tmp1, j);
      }
    }


    // Update q_polynomials according to discrepancies
    if(RQC_ffi_elt_is_zero(RQC_ffi_vec_get_coeff(u1, i)) != 1) {
      updateType = 1;
      ...
      RQC_ffi_elt_inv(tmp1, RQC_ffi_vec_get_coeff(u1, i));
      RQC_ffi_elt_sqr(e1, RQC_ffi_vec_get_coeff(u1, i));
      RQC_ffi_elt_mul(e1, e1, tmp1);
      RQC_ffi_elt_mul(e2, RQC_ffi_vec_get_coeff(u0, i), tmp1);
    ...
    if(RQC_ffi_elt_is_zero(RQC_ffi_vec_get_coeff(u0, i)) == 1 && 
       RQC_ffi_elt_is_zero(RQC_ffi_vec_get_coeff(u1, i)) == 1) {
      updateType = 2;
    ...
    for(unsigned int k = i + 1 ; k < gc.n ; ++k) {
      if(updateType == 1) {
        ...
        RQC_ffi_elt_mul(tmp1, e1, RQC_ffi_vec_get_coeff(u1, k));
        RQC_ffi_elt_sqr(tmp2, RQC_ffi_vec_get_coeff(u1, k));
        RQC_ffi_elt_add(tmp1, tmp1, tmp2);

        RQC_ffi_elt_mul(tmp2, e2, RQC_ffi_vec_get_coeff(u1, k));
        RQC_ffi_elt_add(tmp2, tmp2, RQC_ffi_vec_get_coeff(u0, k));
        RQC_ffi_vec_set_coeff(u1, tmp2, k);

        RQC_ffi_vec_set_coeff(u0, tmp1, k);
      } 
      ...
      if(updateType == 2) {
        ...
        RQC_ffi_elt_sqr(tmp1, u1[k]);
        RQC_ffi_vec_set_coeff(u1, tmp1, k);
      }
    }
  ...
  RQC_ffi_vec_set(m, qtmp1.values, gc.k);

  #ifdef VERBOSE
    printf("\nquotient: "); q_polynomial_print(qtmp1);
    printf("\nremainder: "); q_polynomial_print(qtmp2);
    printf("\nmu: "); RQC_ffi_vec_print(m, RQC_PARAM_K);
