...
q_polynomial q_polynomial_init(unsigned int max_degree) {
  ...
  RQC_ffi_vec_set_length(p.values, max_degree);
...
void q_polynomial_update_degree(q_polynomial& p, unsigned int position) {
  for(int i = position ; i >= 0 ; --i) {
    if(RQC_ffi_elt_is_zero(p.values[i]) == 0) { 
      p.degree = i;
      return;
    }
  }
...
int q_polynomial_set(q_polynomial& o, const q_polynomial& p) {
  ...
  if(q_polynomial_is_zero(p) == 1) {
    ...
    for(int i = 0 ; i <= p.degree ; ++i) {
      RQC_ffi_vec_set_coeff(o.values, p.values[i], i);
    }

    for(int i = p.degree + 1 ; i <= o.max_degree ; ++i) {
      RQC_ffi_vec_set_coeff(o.values, RQC_ffi_elt_get_zero(), i);
    }
  }
...
int q_polynomial_set_zero(q_polynomial& o) {
  o.degree = -1;
  RQC_ffi_vec_set_zero(o.values, o.max_degree + 1);
...
int q_polynomial_set_one(q_polynomial& o) {
  ...
  RQC_ffi_vec_set_coeff(o.values, RQC_ffi_elt_get_one(), 0);
 ...
 */
int q_polynomial_set_interpolate_vect_and_zero(q_polynomial& o1, q_polynomial& o2, const RQC_ffi_vec& v1, const RQC_ffi_vec& v2, int size) {
  if(o1.max_degree < size || o2.max_degree < size - 1) {
    return INVALID_PARAMETERS;
  }

  RQC_ffi_elt tmp1, tmp2, tmp3;
  ...
  for(unsigned int i = 0 ; i < size ; ++i)  {
    ...
    RQC_ffi_elt_inv(tmp3, tmp1);
    RQC_ffi_elt_add(tmp2, tmp2, v2[i]);
    RQC_ffi_elt_mul(tmp2, tmp2, tmp3);
 ...
 */
void q_polynomial_evaluate(RQC_ffi_elt& o, const q_polynomial& p, const RQC_ffi_elt& e) {
  if(q_polynomial_is_zero(p) == 1) {
    RQC_ffi_elt_set_zero(o);
  } 
  else {
    RQC_ffi_elt tmp1, tmp2;

    RQC_ffi_elt_set(tmp1, e);
    RQC_ffi_elt_mul(o, p.values[0], tmp1);

    for(int i = 1 ; i <= p.degree ; ++i) {
      RQC_ffi_elt_sqr(tmp1, tmp1);
      RQC_ffi_elt_mul(tmp2, p.values[i], tmp1);
      RQC_ffi_elt_add(o, o, tmp2);
    }
  }
}
 ...
 */
int q_polynomial_scalar_mul(q_polynomial& o, const q_polynomial& p, const RQC_ffi_elt& e) {
  if(o.max_degree < p.degree) {
    return INVALID_PARAMETERS;
  }

  if(q_polynomial_is_zero(p) == 1 || RQC_ffi_elt_is_zero(e) == 1) {
    q_polynomial_set_zero(o);
  } else {

    RQC_ffi_elt tmp;
    for(int i = 0 ; i <= p.degree ; ++i) {
      RQC_ffi_elt_mul(tmp, p.values[i], e);
      RQC_ffi_vec_set_coeff(o.values, tmp, i);
    }
...
int q_polynomial_qexp(q_polynomial& o, const q_polynomial& p) {
  ...
  if(q_polynomial_is_zero(p) == 1) {
    ...
    RQC_ffi_elt tmp;
    RQC_ffi_vec_set_coeff(o.values, RQC_ffi_elt_get_zero(), 0);
    for(int i = 0 ; i <= p.degree ; ++i) {
      RQC_ffi_elt_sqr(tmp, p.values[i]);
      RQC_ffi_vec_set_coeff(o.values, tmp, (i+1) % RQC_ffi_field_get_degree());
    }
...
int q_polynomial_add(q_polynomial& o, const q_polynomial& p, const q_polynomial& q) {
  ...
  if(degree == -1) {
    ...
    for(int i = 0 ; i <= o.degree ; ++i) {
      RQC_ffi_elt_add(tmp, RQC_ffi_vec_get_coeff(p.values, i), RQC_ffi_vec_get_coeff(q.values, i));
      RQC_ffi_vec_set_coeff(o.values, tmp, i); 
    }
...
int q_polynomial_mul(q_polynomial& o, const q_polynomial& p, const q_polynomial& q) {
  ...
  if(q_polynomial_is_zero(p) == 1 || q_polynomial_is_zero(q) == 1) {
    ...
    for(int j = 0 ; j <= q.degree ; ++j) {
      RQC_ffi_elt_set(tmp1, q.values[j]);
      RQC_ffi_elt_mul(tmp2, p.values[0], tmp1);
      RQC_ffi_elt_add(tmp3, o.values[j], tmp2);
      RQC_ffi_vec_set_coeff(o.values, tmp3, j);
      for(int i = 1 ; i <= p.degree ; ++i) {
        RQC_ffi_elt_sqr(tmp1, tmp1);
        RQC_ffi_elt_mul(tmp2, p.values[i], tmp1);
        RQC_ffi_elt_add(tmp3, RQC_ffi_vec_get_coeff(o.values, (i+j) % RQC_ffi_field_get_degree()), tmp2); 
        RQC_ffi_vec_set_coeff(o.values, tmp3, (i+j) % RQC_ffi_field_get_degree());
      }
    }
...
int q_polynomial_left_div(q_polynomial& q, q_polynomial& r, const q_polynomial& a, const q_polynomial& b) {
  ...
  RQC_ffi_elt b_lead_coeff_inv;
  RQC_ffi_elt r_lead_coeff;
  RQC_ffi_elt s_lead_coeff;
  
  RQC_ffi_elt_set(b_lead_coeff_inv, b.values[b.degree]);
  RQC_ffi_elt_inv(b_lead_coeff_inv, b_lead_coeff_inv);

  while(rtmp.degree >= b.degree) {
    // Compute monial s
    RQC_ffi_elt_set(r_lead_coeff, rtmp.values[rtmp.degree]);
    RQC_ffi_elt_mul(s_lead_coeff, r_lead_coeff, b_lead_coeff_inv);
    RQC_ffi_elt_nth_root(s_lead_coeff, s_lead_coeff, b.degree);

    q_polynomial_set_zero(s);
    RQC_ffi_vec_set_coeff(s.values, s_lead_coeff, rtmp.degree - b.degree);
...
void q_polynomial_print(const q_polynomial& p) {
  if(q_polynomial_is_zero(p) == 1) {
    ...
    for(int i = p.degree; i > 0 ; --i) {
      RQC_ffi_elt_print(p.values[i]);
      printf("x^[%i] + ", i);
    }
    RQC_ffi_elt_print(p.values[0]);
    printf("x^[0]");
  }
  printf("\n");
}
