/**
 * \file RQC_ffi_vec.cpp
 * \brief Implementation of RQC_ffi_vec.h using the NTL library
 */
...
static unsigned int RQC_ffi_vec_get_rank(RQC_ffi_vec v, unsigned int size);
...
/** 
 * \fn RQC_ffi_elt RQC_ffi_vec_get_coeff(const RQC_ffi_vec& v, unsigned int position)
...
/** 
 * \fn void RQC_ffi_vec_set_coeff(RQC_ffi_vec& o, const RQC_ffi_elt& e, unsigned int position)
...
/** 
 * \fn void RQC_ffi_vec_set_length(RQC_ffi_vec& o, unsigned int size)
...
/** 
 * \fn void RQC_ffi_vec_set(RQC_ffi_vec& o, const RQC_ffi_vec& v, unsigned int size)
...
/** 
 * \fn void RQC_ffi_vec_set_zero(RQC_ffi_vec& v, unsigned int size)
...
/** 
 * \fn void RQC_ffi_vec_set_random(RQC_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx)
...
void RQC_ffi_vec_set_random(RQC_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx) {
  RQC_ffi_vec_set_length(o, size);
	for(unsigned int i = 0 ; i < size ; ++i) {
		RQC_ffi_elt_set_random(o[i], ctx);
	}
}
...
/** 
 * \fn void RQC_ffi_vec_set_random2(RQC_ffi_vec& o, unsigned int size)
...
/** 
 * \fn void RQC_ffi_vec_set_random_full_rank(RQC_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx)
...
void RQC_ffi_vec_set_random_full_rank(RQC_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx) {
  unsigned int rank_max = RQC_FIELD_M < size ? RQC_FIELD_M : size;

  unsigned int rank = -1;
  while(rank != rank_max) {
    RQC_ffi_vec_set_random(o, size, ctx);
    rank = RQC_ffi_vec_get_rank(o, size);
  }
}
...
/** 
 * \fn void RQC_ffi_vec_set_random_full_rank_with_one(RQC_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx)
...
void RQC_ffi_vec_set_random_full_rank_with_one(RQC_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx) {
  ...
  unsigned int rank = -1;
  while(rank != rank_max) {
    RQC_ffi_vec_set_random(o, size - 1, ctx);
    RQC_ffi_vec_set_coeff(o, RQC_ffi_elt_get_one(), size - 1);
    rank = RQC_ffi_vec_get_rank(o, size);
  }
}
...
/** 
 * \fn void RQC_ffi_vec_set_random_from_support(RQC_ffi_vec& o, unsigned int size, const RQC_ffi_vec& support, unsigned int rank, AES_XOF_struct* ctx)
...
void RQC_ffi_vec_set_random_from_support(RQC_ffi_vec& o, unsigned int size, const RQC_ffi_vec& support, unsigned int rank, AES_XOF_struct* ctx) {
  ...
  while(i != rank) {
    ...
    if(position < size * (256 / size) && RQC_ffi_elt_is_zero(o[position % size]) == 1) {
      RQC_ffi_vec_set_coeff(o, support[i], position % size);
      i++;
    }
  ...
  int k = 0;
  for(i = 0 ; i < size ; ++i) {
    if(RQC_ffi_elt_is_zero(o[i])) {
      for(j = 0 ; j < rank ; ++j) {
        if((random_lin_comb[k * rank + j] % RQC_FIELD_Q) == 1) {
          RQC_ffi_elt_add(o[i], support[j], o[i]);
        }
      }
      k++;
    }
...
/** 
 * \fn unsigned int RQC_ffi_vec_get_rank(RQC_ffi_vec v, unsigned int size)
...
/** 
 * \fn int RQC_ffi_vec_is_equal_to(const RQC_ffi_vec& v1, const RQC_ffi_vec& v2, unsigned int size)
 ...
 */
int RQC_ffi_vec_is_equal_to(const RQC_ffi_vec& v1, const RQC_ffi_vec& v2, unsigned int size) {
  return v1 == v2;
}
...
/** 
 * \fn void RQC_ffi_vec_add(RQC_ffi_vec& o, const RQC_ffi_vec& v1, const RQC_ffi_vec& v2, unsigned int size)
...
/** 
 * \fn void RQC_ffi_vec_mul(RQC_ffi_vec& o, const RQC_ffi_vec& v1, const RQC_ffi_vec& v2, unsigned int size)
...
void RQC_ffi_vec_mul(RQC_ffi_vec& o, const RQC_ffi_vec& v1, const RQC_ffi_vec& v2, unsigned int size) {
	MulMod(o, v1, v2, RQC_ideal_modulo);
}
...
/** 
 * \fn void RQC_ffi_vec_to_string(unsigned char* str, const RQC_ffi_vec& v, unsigned int size)
 * \brief This function parses a vector into a string
 *
 * This function stores the <b>RQC_FIELD_N * RQC_FIELD_M</b> bits required to describe <b>v</b> into a string.
...
void RQC_ffi_vec_to_string(unsigned char* str, const RQC_ffi_vec& v, unsigned int size) {
  size = deg(v);
	for(unsigned int i = 0 ; i <= size ; i++) {
		RQC_ffi_elt_to_string(str + i * RQC_FIELD_ELT_BYTES, v[i]);
	}
}
...
/** 
 * \fn void RQC_ffi_vec_to_string_compact(unsigned char* str, const RQC_ffi_vec& v, unsigned int size)
 * \brief This function parses a vector into a string
 *
 * This function stores the <b>RQC_FIELD_N * RQC_FIELD_M</b> bits required to describe <b>v</b> into the smallest possible string.
 *
 * \param[out] str Output string
 * \param[in] v Vector to parse
 * \param[in] size Size of the vector
...
void RQC_ffi_vec_to_string_compact(unsigned char* str, const RQC_ffi_vec& v, unsigned int size) {
  unsigned int size2 = (RQC_FIELD_M % 8 == 0) ? size * RQC_FIELD_M / 8 : size * RQC_FIELD_M / 8 + 1;
  memset(str, 0, size2);
  ...
  for(position1 = 0 ; position1 < size ; position1++) {
    for(position2 = 0 ; position2 < RQC_FIELD_M ; position2++) {
...
/** 
 * \fn void RQC_ffi_vec_from_string(RQC_ffi_vec& v, unsigned int size, const unsigned char* str)
 * \brief This function parses a string into a vector
 *
 * The input string is expected to be an uncompacted string generated by the RQC_ffi_vec_to_string function.
 *
 * \param[out] v Output vector
 * \param[in] size Size of the vector <b>v</b>
 * \param[in] str String to parse
 */
void RQC_ffi_vec_from_string(RQC_ffi_vec& v, unsigned int size, const unsigned char *str) {
  RQC_ffi_vec_set_length(v, size);
	for(unsigned int i = 0 ; i < size ; i++) {
		RQC_ffi_elt_from_string(v[i], str + i * RQC_FIELD_ELT_BYTES);
	}
}
...
/** 
 * \fn void RQC_ffi_vec_from_string_compact(RQC_ffi_vec& v, unsigned int size, const unsigned char* str)
 * \brief This function parses a string into a vector
 *
 * The input string is expected to be a compacted string generated by the RQC_ffi_vec_to_string_compact function.
 *
 * \param[out] v Output vector
 * \param[in] size Size of the vector <b>v</b>
 * \param[in] str String to parse
 */
void RQC_ffi_vec_from_string_compact(RQC_ffi_vec &v, unsigned int size, const unsigned char* str) {
  unsigned int position = 0;
  unsigned int position1 = 0;
  unsigned int position2 = 0;
...
/** 
 * \fn void RQC_ffi_vec_print(const RQC_ffi_vec& v, unsigned int size)
