...
int genpoly_gen(gf *out, gf *f)
{
	...
	for (i = 1; i < CM_SYS_T; i++)
		mat[0][i] = 0;

	for (i = 0; i < CM_SYS_T; i++)
		mat[1][i] = f[i];

	for (j = 2; j <= CM_SYS_T; j++)
	...
	{
		for (k = j + 1; k < CM_SYS_T; k++)
		{
			mask = gf_iszero(mat[ j ][ j ]);

			for (c = j; c < CM_SYS_T + 1; c++)
...
{
	int i;
	uint64_t list[1 << CM_GFBITS];

	for (i = 0; i < (1 << CM_GFBITS); i++)
		list[i] = p[i];
        
	sort_63b(1 << CM_GFBITS, list);
        
	for (i = 1; i < (1 << CM_GFBITS); i++)
		if (list[i-1] == list[i])
			return -1;
