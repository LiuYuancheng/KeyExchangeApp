...
void GF_mul(gf *out, gf *in0, gf *in1)
{
	...
	gf prod[ CM_SYS_T*2-1 ];

	for (i = 0; i < CM_SYS_T*2-1; i++)
		prod[i] = 0;
	...
	for (i = (CM_SYS_T-1)*2; i >= CM_SYS_T; i--)
	{
		prod[i - CM_SYS_T +  2] ^= gf_mul(prod[i], (gf) 6400);
		prod[i - CM_SYS_T +  0] ^= gf_mul(prod[i], (gf) 3134);
	}
