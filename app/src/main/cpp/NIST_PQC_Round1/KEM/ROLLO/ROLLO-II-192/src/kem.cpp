...
int crypto_kem_enc(unsigned char* ct, unsigned char* ss, const unsigned char* pk) {
  ...
  memcpy(mc + SHA512_BYTES + FFI_VEC_N_BYTES, c_tmp.v, SHA512_BYTES);
...
int crypto_kem_dec(unsigned char* ss, const unsigned char* ct, const unsigned char* sk) {
  ...
  //d = d' ?
  if(memcmp(c.d, c2.d, SHA512_BYTES)) retValue = 1;
  //v = v' ?
  if(memcmp(c.v, c2.v, SHA512_BYTES)) retValue = 1;
  //u = u' ?
  if(!ffi_vec_cmp(c.u, c2.u)) retValue = 1;
  ...
  if(retValue == 0) {
	...
	memcpy(mc + SHA512_BYTES + FFI_VEC_N_BYTES, c.v, SHA512_BYTES);
	sha512(ss, mc, CIPHERTEXT_BYTES);
  } else {
  ...
  #ifdef VERBOSE
    printf("\n\nsk(seed): "); for(int i = 0 ; i < SEEDEXPANDER_SEED_BYTES ; ++i) printf("%02x", sk[i]);
    printf("\n\nF: "); ffi_vec_print(sk_tmp.F, PARAM_D);
    printf("\n\nx: "); ffi_vec_print(sk_tmp.x, PARAM_N);
    printf("\n\ny: "); ffi_vec_print(sk_tmp.y, PARAM_N);
    printf("\n\nu(syndrom): "); ffi_vec_print(c.u, PARAM_N);
    printf("\n\nv: "); for(int i = 0 ; i < CRYPTO_BYTES ; ++i) printf("%02x", c.v[i]);
    printf("\n\nd: "); for(int i = 0 ; i < CRYPTO_BYTES ; ++i) printf("%02x", c.d[i]);
