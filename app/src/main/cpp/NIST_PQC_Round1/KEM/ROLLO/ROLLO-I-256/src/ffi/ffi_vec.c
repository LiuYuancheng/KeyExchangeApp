/**
 * \file ROLLO_ffi_vec.cpp
 * \brief Implementation of ROLLO_ffi_vec.h using NTL library
 */
...
/** 
 * \fn void ROLLO_ffi_vec_set(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v, unsigned int size)
...
/** 
 * \fn void ROLLO_ffi_vec_set_zero(ROLLO_ffi_vec& v, unsigned int size)
...
/** 
 * \fn void ROLLO_ffi_vec_set_random_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx)
...
void ROLLO_ffi_vec_set_random_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx) {
	...
	for(unsigned int i = 0 ; i < size ; ++i) {
		ROLLO_ffi_elt_set_random_using_seedexpander(o[i], ctx);
	}
}


/** 
 * \fn void ROLLO_ffi_vec_set_random_full_rank_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx)
...
void ROLLO_ffi_vec_set_random_full_rank_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx) {
  unsigned int rank_max = ROLLO_FIELD_M < size ? ROLLO_FIELD_M : size;

  unsigned int rank = -1;
  while(rank != rank_max) {
    ROLLO_ffi_vec_set_random_using_seedexpander(o, size, ctx);
    rank = ROLLO_ffi_vec_get_rank(o, size);
  }
}

/** 
 * \fn void ROLLO_ffi_vec_set_random_full_rank_with_one_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx)
 * \brief This function sets a vector with random values using the NIST seed expander. In addition, the vector returned by this function has full rank and contains one.
 *
...
void ROLLO_ffi_vec_set_random_full_rank_with_one_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, AES_XOF_struct* ctx) {
  ...
  ROLLO_ffi_elt one;
  ROLLO_ffi_elt_set_one(one);

  unsigned int rank = -1;
  while(rank != size) {
    ROLLO_ffi_vec_set_random_using_seedexpander(o, size - 1, ctx);
    ROLLO_ffi_vec_set_coeff(o, one, size - 1);
    rank = ROLLO_ffi_vec_get_rank(o, size);
  }
}
...
/** 
 * \fn void ROLLO_ffi_vec_set_random_from_support_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, const ROLLO_ffi_vec& support, unsigned int rank, AES_XOF_struct* ctx)
...
void ROLLO_ffi_vec_set_random_from_support_using_seedexpander(ROLLO_ffi_vec& o, unsigned int size, const ROLLO_ffi_vec& support, unsigned int rank, AES_XOF_struct* ctx) {
  ...
  while(i != rank) {
    ...
    if(position < size * (256 / size) && ROLLO_ffi_elt_is_zero(coeff(o, position % size)) == 1) {
      ROLLO_ffi_vec_set_coeff(o, support[i], position % size);
      i++;
    }
  ...
  int k = 0;
  for(i = 0 ; i < size ; ++i) {
    if(ROLLO_ffi_elt_is_zero(coeff(o, i))) {
      for(j = 0 ; j < rank ; ++j) {
        if((random_lin_comb[k * rank + j] % ROLLO_FIELD_Q) == 1) {
          ROLLO_ffi_elt tmp;
          ROLLO_ffi_elt_add(tmp, coeff(support, j), coeff(o, i));
          ROLLO_ffi_vec_set_coeff(o, tmp, i);
        }
      }
      k++;
    }
...
/** 
 * \fn void ROLLO_ffi_vec_set_random_using_rng(ROLLO_ffi_vec& o, unsigned int size)
 ...
 * \param[in] ctx NIST seed expander
 */
void ROLLO_ffi_vec_set_random_using_rng(ROLLO_ffi_vec& o, unsigned int size) {
	o.SetLength(size);
	for(unsigned int i = 0 ; i < size ; ++i) {
		ROLLO_ffi_elt_set_random_using_rng(o[i]);
	}
}
...
/** 
 * \fn void ROLLO_ffi_vec_set_random_full_rank_using_rng(ROLLO_ffi_vec& o, unsigned int size)
...
void ROLLO_ffi_vec_set_random_full_rank_using_rng(ROLLO_ffi_vec& o, unsigned int size) {
  ...
  unsigned int rank = -1;
  while(rank != rank_max) {
	  ROLLO_ffi_vec_set_random_using_rng(o, size);
    rank = ROLLO_ffi_vec_get_rank(o, size);
	}
}
...
void ROLLO_ffi_vec_set_random_full_rank_with_one_using_rng(ROLLO_ffi_vec& o, unsigned int size) {
  ...
  unsigned int rank = -1;
  while(rank != size) {
    ROLLO_ffi_vec_set_random_using_rng(o, size - 1);
    ROLLO_ffi_vec_set_coeff(o, one, size - 1);
    rank = ROLLO_ffi_vec_get_rank(o, size);
  }
}
...
/** 
 * \fn void ROLLO_ffi_vec_set_random_from_support_using_rng(ROLLO_ffi_vec& o, unsigned int size, const ROLLO_ffi_vec& support, unsigned int rank)
 * \brief This function sets a vector with random values using the NIST seed expander. In addition, the support of the vector returned by this function is the one given in input.
 *
 * This function copies the support vector in rank random positions of <b>v</b>. Next, all the remaining coordinates of <b>v</b> are set using random linear combinations of the support coordinates.
 *
...
/** 
 * \fn void ROLLO_ffi_vec_set_coeff(ROLLO_ffi_vec& o, const ROLLO_ffi_elt& e, unsigned int position)
 ...
 */
void ROLLO_ffi_vec_set_coeff(ROLLO_ffi_vec &o, const ROLLO_ffi_elt& e, unsigned int position) {
...
/** 
 * \fn void ROLLO_ffi_vec_get_coeff(ROLLO_ffi_elt& o, const ROLLO_ffi_vec& v, unsigned int position)
...
/**
 * @brief      Test equality of two ROLLO_ffi_vec
 *
...
/** 
 * \fn void ROLLO_ffi_vec_echelonize(ROLLO_ffi_vec& o, unsigned int size)
...
void ROLLO_ffi_vec_echelonize(ROLLO_ffi_vec &o, unsigned int size) {
	int ref_size = ROLLO_ffi_vec_gauss(o, size);
...
/** 
 * \fn unsigned int ROLLO_ffi_vec_gauss(ROLLO_ffi_vec& o, unsigned int size)
...
unsigned int ROLLO_ffi_vec_gauss(ROLLO_ffi_vec& o, unsigned int size) {
  ...
  ROLLO_ffi_elt zeroElt;
  ROLLO_ffi_elt_set_zero(zeroElt);

  vec_GF2 tmp;
	for(unsigned int i = 0 ; i < size ; ++i) {
    conv(tmp, rep(o[i]));
    tmp.SetLength(ROLLO_FIELD_M);
		vectors[i] = tmp;
	}
...
/** 
 * \fn unsigned int ROLLO_ffi_vec_get_rank(ROLLO_ffi_vec v, unsigned int size)
...
unsigned int ROLLO_ffi_vec_get_rank(ROLLO_ffi_vec v, unsigned int size) {
	return ROLLO_ffi_vec_gauss(v, size);
}



/** 
 * \fn void ROLLO_ffi_vec_add(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, const ROLLO_ffi_vec& v2, unsigned int size)
...
/** 
 * \fn void ROLLO_ffi_vec_mul(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, const ROLLO_ffi_vec& v2, unsigned int size)
...
void ROLLO_ffi_vec_mul(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, const ROLLO_ffi_vec& v2, unsigned int size) {
	MulMod(o, v1, v2, ROLLO_ideal_modulo);
}



/** 
 * \fn void ROLLO_ffi_vec_scalar_mul(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v, const ROLLO_ffi_elt& e, unsigned int size)
...
/** 
 * \fn void ROLLO_ffi_vec_tensor_mul(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, unsigned int size1, const ROLLO_ffi_vec& v2, unsigned int size2)
...
void ROLLO_ffi_vec_tensor_mul(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, unsigned int size1, const ROLLO_ffi_vec& v2, unsigned int size2) {
	...
	for(unsigned int i = 0 ; i < size1 ; ++i) {
		for(unsigned int j = 0 ; j < size2 ; ++j) {
			ROLLO_ffi_elt_mul(o[i * size2 + j], v1[i], v2[j]);
		}
	}
}
...
/** 
 * \fn void ROLLO_ffi_vec_directsum(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, unsigned int size1, const ROLLO_ffi_vec& v2, unsigned int size2)
...
void ROLLO_ffi_vec_directsum(ROLLO_ffi_vec& o, const ROLLO_ffi_vec& v1, unsigned int size1, const ROLLO_ffi_vec& v2, unsigned int size2) {
	...
	for(unsigned int i = 0 ; i < size1 ; ++i) {
		ROLLO_ffi_elt_set(o[i], v1[i]);
	}

	for(unsigned int i = 0 ; i < size2 ; ++i) {
		ROLLO_ffi_elt_set(o[size1 + i], v2[i]);
	}
}
...
/** 
 * \fn void ROLLO_ffi_vec_intersection(ROLLO_ffi_vec& o, unsigned int& size, const ROLLO_ffi_vec& v1, unsigned int size1, const ROLLO_ffi_vec& v2, unsigned int size2)
...
void ROLLO_ffi_vec_intersection(ROLLO_ffi_vec& o, unsigned int& size, const ROLLO_ffi_vec& v1, unsigned int size1, const ROLLO_ffi_vec& v2, unsigned int size2) {
  ...
  // Extract the basis of the intersection
	...
	for(unsigned int i = 0 ; i < size1 + size2 ; ++i) {
		shift(tmp, matrix[i], ROLLO_FIELD_M);
...
/** 
 * \fn void ROLLO_ffi_vec_to_string(unsigned char* str2, const ROLLO_ffi_vec v, unsigned int size)
 * \brief This function parses a vector into a string
 *
 * This function only copies the bits of <b>v</b> containing information into <b>str2</b>. Indeed, there are only <b>ROLLO_FIELD_M</b> of them while the ROLLO_ffi_vec bit representation is larger due to library storing elements differently.
 *
 * \param[out] str2 Output string
 * \param[in] v Vector to parse
 * \param[in] size Size of the vector
 */
void ROLLO_ffi_vec_to_string(unsigned char* str2, const ROLLO_ffi_vec &v, unsigned int size) {
  unsigned int size2 = (ROLLO_FIELD_M % 8 == 0) ? size * ROLLO_FIELD_M / 8 : size * ROLLO_FIELD_M / 8 + 1;
  ...
  for(position1 = 0 ; position1 < size ; position1++) {
    for(position2 = 0 ; position2 < ROLLO_FIELD_M ; position2++) {
...
/** 
 * \fn void ROLLO_ffi_vec_from_string(ROLLO_ffi_vec v, unsigned int size, const unsigned char* str2)
 * \brief This function parses a string into a vector
 *
 * This function turns the <b>ROLLO_FIELD_M</b> bits of information required to describe <b>v</b> into a ROLLO_ffi_vec bit representation which is larger due to MPFQ storing elements as pointer of unsigned longs.
 *
 * \param[out] v Output vector
 * \param[in] size Size of the vector <b>v</b>
 * \param[in] str2 String to parse
 */
void ROLLO_ffi_vec_from_string(ROLLO_ffi_vec &v, unsigned int size, const unsigned char* str2) {
  unsigned int position = 0;
  unsigned int position1 = 0;
  unsigned int position2 = 0;
...
/** 
 * \fn void ROLLO_ffi_vec_print(const ROLLO_ffi_vec& v, unsigned int size)
...
long ROLLO_ffi_vec_inv(ROLLO_ffi_vec &inv, ROLLO_ffi_vec A) {
  A.normalize();
  return InvModStatus(inv, A, ROLLO_ideal_modulo);
}
