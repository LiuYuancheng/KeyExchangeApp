...
/** 
 * \fn static void expand_space(ROLLO_ffi_vec &S, unsigned int &S_dim, unsigned int E_expected_dim, ROLLO_ffi_vec F, unsigned int F_dim)
 ...
 */
static void expand_space(ROLLO_ffi_vec &S, unsigned int &S_dim, unsigned int E_expected_dim, ROLLO_ffi_vec F,  unsigned int F_dim) {
  // Compute the spaces Si = F[i]^(-1) * S
  ROLLO_ffi_vec Si[F_dim];
  ROLLO_ffi_elt Fi_inv;
  unsigned int Si_dim = S_dim;
  for(unsigned int i = 0 ; i < F_dim ; ++i) {
    ROLLO_ffi_elt_inv(Fi_inv, F[i]);
    ROLLO_ffi_vec_scalar_mul(Si[i], S, Fi_inv, Si_dim);
  }

  // Compute the spaces inters[i] as the intersection between Si and Si+1
  ROLLO_ffi_vec inters[F_dim-1];
  unsigned int inters_dim[F_dim-1];
  for(unsigned int i = 0 ; i < F_dim - 1 ; ++i) {
    ROLLO_ffi_vec_intersection(inters[i], inters_dim[i], Si[i], Si_dim, Si[i+1], Si_dim);
  }

  // Expand S until its dimension is F_dim * E_expected_dim
  ROLLO_ffi_vec inter, dsum1, dsum2, add, tmp1, tmp2;
  unsigned int inter_dim, dsum2_dim, tmp1_dim, tmp2_dim;
  for(unsigned int i = 0 ; i < F_dim - 2 ; ++i) {
    ROLLO_ffi_vec_intersection(inter, inter_dim, Si[i], Si_dim, Si[i+2], Si_dim);

    ROLLO_ffi_vec_directsum(dsum1, inters[i], inters_dim[i], inters[i+1], inters_dim[i+1]);
    ROLLO_ffi_vec_directsum(dsum2, dsum1, inters_dim[i] + inters_dim[i+1], inter, inter_dim);
    dsum2_dim = ROLLO_ffi_vec_gauss(dsum2, inters_dim[i] + inters_dim[i+1] + inter_dim);

    ROLLO_ffi_vec_tensor_mul(add, F, F_dim, dsum2, dsum2_dim);
    ROLLO_ffi_vec_directsum(tmp1, S, S_dim, add, F_dim * dsum2_dim);
    tmp1_dim = ROLLO_ffi_vec_gauss(tmp1, S_dim + F_dim * dsum2_dim);

    if(tmp1_dim > F_dim * E_expected_dim) continue;
    else {
      ROLLO_ffi_vec_directsum(tmp2, S, S_dim, tmp1, tmp1_dim);
      tmp2_dim = ROLLO_ffi_vec_gauss(tmp2, S_dim + tmp1_dim);
      ROLLO_ffi_vec_set(S, tmp2, tmp2_dim);
      S_dim = tmp2_dim;
    }
  }
}
...
/** 
 * \fn void rank_support_recover(ROLLO_ffi_vec& E, unsigned int& E_dim, unsigned int E_expected_dim, const ROLLO_ffi_vec& F, unsigned int F_dim, const ROLLO_ffi_vec& ec, unsigned int ec_size)
...
void rank_support_recover(ROLLO_ffi_vec& E, unsigned int& E_dim, unsigned int E_expected_dim, const ROLLO_ffi_vec& F, unsigned int F_dim, const ROLLO_ffi_vec& ec, unsigned int ec_size) {
  ...
  ROLLO_ffi_vec S;
  ROLLO_ffi_vec_set(S, ec, ec_size);
  unsigned int S_dim = ROLLO_ffi_vec_gauss(S, ec_size);

  #ifdef VERBOSE
    printf("\n\n# Rank Support Recover - Begin #");
    printf("\n\nS: "); ROLLO_ffi_vec_print(S, S_dim);
  #endif
  ...
  for(unsigned int i = 0 ; i < F_dim ; ++i) {
    ROLLO_ffi_elt_inv(Fi_inv, F[i]);
    ROLLO_ffi_vec_scalar_mul(Si[i], S, Fi_inv, S_dim);
  }

  // Retrieve the support E by computing the intersection of the Si spaces (line 10)
  ROLLO_ffi_vec_intersection(E, E_dim, Si[0], S_dim, Si[1], S_dim);

  if(E_dim > E_expected_dim) {
    for(unsigned int i = 2 ; i < F_dim ; ++i) {
      ROLLO_ffi_vec_intersection(E, E_dim, E, E_dim, Si[i], S_dim); 
    ...
    printf("\n\nS: "); ROLLO_ffi_vec_print(S, S_dim);
    for(unsigned int i = 0 ; i < F_dim ; ++i) {
      printf("\n\nS%i: ", i); ROLLO_ffi_vec_print(Si[i], S_dim);
    }
    printf("\n\nE: "); ROLLO_ffi_vec_print(E, E_dim);
  #endif

  ROLLO_ffi_vec_echelonize(E, E_dim);
