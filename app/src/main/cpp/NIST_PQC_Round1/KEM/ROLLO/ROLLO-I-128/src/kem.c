...
int ROLLO_crypto_kem_keypair(unsigned char* pk, unsigned char* sk) {
  ...
  ROLLO_ffi_vec_mul(pk_tmp.h, invx, sk_tmp.y, PARAM_N);
...
int ROLLO_crypto_kem_enc(unsigned char* ct, unsigned char* ss, const unsigned char* pk) {
  ...
  ffi_vec E;
  ROLLO_ffi_vec_set_random_full_rank_using_rng(E, PARAM_R);

  //Polynomials
  ffi_vec polyE1, polyE2, polyM;
  ROLLO_ffi_vec_set_random_from_support_using_rng(polyE1, PARAM_N, E, PARAM_R);
  ROLLO_ffi_vec_set_random_from_support_using_rng(polyE2, PARAM_N, E, PARAM_R);

  //Compute ciphertext
  ciphertext c_tmp;

  //Left part : we multiply with the generator matrix by the message and then add right part of the error
  ROLLO_ffi_vec_mul(c_tmp.syndrom, polyE2, pk_tmp.h, PARAM_N);
  ROLLO_ffi_vec_add(c_tmp.syndrom, c_tmp.syndrom, polyE1, PARAM_N);

  lake_ciphertext_to_string(ct, c_tmp.syndrom);

  ROLLO_ffi_vec_echelonize(E, PARAM_R);

  // Compute shared secret
  
  unsigned char support[FFI_VEC_R_BYTES];
  ROLLO_ffi_vec_to_string(support, E, PARAM_R);
  sha512(ss, support, FFI_VEC_R_BYTES);

  #ifdef VERBOSE
    printf("\n\nh: "); ffi_vec_print(pk_tmp.h, PARAM_N);
...
int ROLLO_crypto_kem_dec(unsigned char* ss, const unsigned char* ct, const unsigned char* sk) {
  ...
  //Syndrom computation
  ffi_vec xc;
  ROLLO_ffi_vec_mul(xc, sk_tmp.x, c.syndrom, PARAM_N);
  #ifdef VERBOSE
  ...
  ffi_vec E;
  unsigned int E_dim;
  rank_support_recover(E, E_dim, PARAM_R, sk_tmp.F, PARAM_D, xc, PARAM_N);
...
//    memset(ss, 0, sizeof(ROLLO_CRYPTO_BYTES));   
    memset(ss, 0, ROLLO_CRYPTO_BYTES);
  }
