...
void lake_secret_key_from_string(secretKey &sk, const unsigned char* skString) {
  ...
  ROLLO_ffi_vec_set_random_full_rank_using_seedexpander(sk.F, PARAM_D, sk_seedexpander);
  ...
  ROLLO_ffi_vec_set_random_from_support_using_seedexpander(sk.x, PARAM_N, sk.F, PARAM_D, sk_seedexpander);
  ROLLO_ffi_vec_set_random_from_support_using_seedexpander(sk.y, PARAM_N, sk.F, PARAM_D, sk_seedexpander);
...
void lake_public_key_to_string(unsigned char* pkString, publicKey pk) {
  ROLLO_ffi_vec_to_string(pkString, pk.h, PARAM_N);
}
...
void lake_public_key_from_string(publicKey &pk, const unsigned char *pkString) {
  ROLLO_ffi_vec_from_string(pk.h, PARAM_N, pkString);
}
...
void lake_ciphertext_to_string(unsigned char* ct, const ffi_vec& c) {
  ROLLO_ffi_vec_to_string(ct, c, PARAM_N);
}
...
void lake_ciphertext_from_string(const unsigned char* ct, ffi_vec &c) {
  ROLLO_ffi_vec_from_string(c, PARAM_N, ct);
}
