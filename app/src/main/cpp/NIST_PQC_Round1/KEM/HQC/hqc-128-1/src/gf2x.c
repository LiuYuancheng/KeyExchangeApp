...
void ntl_cyclic_product(uint8_t*o, const uint8_t* v1, const uint8_t* v2) {
  ...
  GF2XFromBytes(poly1, v1, HQC_VEC_N_SIZE_BYTES);
  GF2XFromBytes(poly2, v2, HQC_VEC_N_SIZE_BYTES);
  GF2XModulus P = init_modulo();

  MulMod(tmp, poly1, poly2, P);

  BytesFromGF2X(o, tmp, HQC_VEC_N_SIZE_BYTES);
}
