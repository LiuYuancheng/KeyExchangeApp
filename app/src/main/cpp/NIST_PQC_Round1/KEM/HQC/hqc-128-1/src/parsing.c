...
void HQC_secret_key_to_string(unsigned char* sk, const unsigned char* sk_seed, const unsigned char* pk) {
  ...
  memcpy(sk + SEED_BYTES, pk, HQC_PUBLIC_KEY_BYTES);
}
...
void HQC_public_key_to_string(unsigned char* pk, const unsigned char* pk_seed, uint8_t* s) {
  ...
  memcpy(pk + SEED_BYTES, s, HQC_VEC_N_SIZE_BYTES);
}
...
void HQC_public_key_from_string(uint8_t* h, uint8_t* s, const unsigned char* pk) {
  ...
  memcpy(s, pk + SEED_BYTES, HQC_VEC_N_SIZE_BYTES);
...
void HQC_ciphertext_to_string(unsigned char* ct, uint8_t* u, uint8_t* v, const unsigned char* d) {
  memcpy(ct, u, HQC_VEC_N_SIZE_BYTES);
  memcpy(ct + HQC_VEC_N_SIZE_BYTES, v, HQC_VEC_N1N2_SIZE_BYTES);
  memcpy(ct + HQC_VEC_N_SIZE_BYTES + HQC_VEC_N1N2_SIZE_BYTES, d, SHA512_BYTES);
}
...
void HQC_ciphertext_from_string(uint8_t* u, uint8_t* v, unsigned char* d, const unsigned char* ct) {
  memcpy(u, ct, HQC_VEC_N_SIZE_BYTES);
  memcpy(v, ct + HQC_VEC_N_SIZE_BYTES, HQC_VEC_N1N2_SIZE_BYTES);
  memcpy(d, ct + HQC_VEC_N_SIZE_BYTES + HQC_VEC_N1N2_SIZE_BYTES, SHA512_BYTES);
}
