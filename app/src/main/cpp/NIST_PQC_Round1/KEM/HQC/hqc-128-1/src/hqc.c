...
void HQC_pke_keygen(unsigned char* pk, unsigned char* sk) {
  ...
  // Compute secret key
  uint8_t x [HQC_VEC_N_SIZE_BYTES] = {0};
  uint8_t y [HQC_VEC_N_SIZE_BYTES] = {0};

  vect_fixed_weight(x, HQC_PARAM_OMEGA, sk_seedexpander);
  vect_fixed_weight(y, HQC_PARAM_OMEGA, sk_seedexpander);

  // Compute public key
  uint8_t h [HQC_VEC_N_SIZE_BYTES] = {0};
  vect_set_random(h, pk_seedexpander);
  uint8_t s [HQC_VEC_N_SIZE_BYTES] = {0};
  ntl_cyclic_product(s, h, y);
  vect_add(s, s, x, HQC_VEC_N_SIZE_BYTES);
  ...
  HQC_secret_key_to_string(sk, sk_seed, pk);
    ...
    printf("\n\nx: "); vect_print(x, HQC_VEC_N_SIZE_BYTES);
    printf("\n\ny: "); vect_print(y, HQC_VEC_N_SIZE_BYTES);

    printf("\n\npk_seed: "); for(int i = 0 ; i < SEED_BYTES ; ++i) printf("%02x", pk_seed[i]);
    printf("\n\nh: "); vect_print(h, HQC_VEC_N_SIZE_BYTES);
    printf("\n\ns: "); vect_print(s, HQC_VEC_N_SIZE_BYTES);

    printf("\n\nsk: "); for(int i = 0 ; i < HQC_SECRET_KEY_BYTES ; ++i) printf("%02x", sk[i]);
    printf("\n\npk: "); for(int i = 0 ; i < HQC_PUBLIC_KEY_BYTES ; ++i) printf("%02x", pk[i]);
  #endif
...
void HQC_pke_encrypt(uint8_t* u, uint8_t* v, uint8_t* m, unsigned char* theta, const unsigned char* pk) {
  ...
  uint8_t r1 [HQC_VEC_N_SIZE_BYTES] = {0};
  uint8_t r2 [HQC_VEC_N_SIZE_BYTES] = {0};
  uint8_t e [HQC_VEC_N_SIZE_BYTES] = {0};
  
  vect_fixed_weight(r1, HQC_PARAM_OMEGA_R, seedexpander);
  vect_fixed_weight(r2, HQC_PARAM_OMEGA_R, seedexpander);
  vect_fixed_weight(e, HQC_PARAM_OMEGA_E, seedexpander); 

  // Compute u = r1 + r2.h
  ntl_cyclic_product(u, r2, h);
  vect_add(u, u, r1, HQC_VEC_N_SIZE_BYTES);

  // Compute v = m.G by encoding the message
  tensor_code_encode(v, m);
  uint8_t tmp1 [HQC_VEC_N_SIZE_BYTES];
  vect_resize(tmp1, HQC_PARAM_N, v, HQC_PARAM_N1N2);

  // Compute v = m.G + s.r2 + e
  uint8_t tmp2 [HQC_VEC_N_SIZE_BYTES];
  ntl_cyclic_product(tmp2, r2 , s);

  vect_add(tmp2, tmp2, e, HQC_VEC_N_SIZE_BYTES);
  vect_add(tmp2, tmp1, tmp2, HQC_VEC_N_SIZE_BYTES);
  vect_resize(v, HQC_PARAM_N1N2, tmp2, HQC_PARAM_N);

  #ifdef VERBOSE
    printf("\n\nh: "); vect_print(h, HQC_VEC_N_SIZE_BYTES);
    printf("\n\ns: "); vect_print(s, HQC_VEC_N_SIZE_BYTES);
    printf("\n\nr1: "); vect_print(r1, HQC_VEC_N_SIZE_BYTES);
    printf("\n\nr2: "); vect_print(r2, HQC_VEC_N_SIZE_BYTES);
    printf("\n\ne: "); vect_print(e, HQC_VEC_N_SIZE_BYTES);
    printf("\n\ntmp2: "); vect_print(tmp2, HQC_VEC_N_SIZE_BYTES);

    printf("\n\nu: "); vect_print(u, HQC_VEC_N_SIZE_BYTES);
    printf("\n\nv: "); vect_print(v, HQC_VEC_N1N2_SIZE_BYTES);
  #endif
...
void HQC_pke_decrypt(uint8_t* m, uint8_t* u, uint8_t* v, const unsigned char* sk) {
  ...
  uint8_t x [HQC_VEC_N_SIZE_BYTES] = {0};
  uint8_t y [HQC_VEC_N_SIZE_BYTES] = {0};
  unsigned char pk[HQC_PUBLIC_KEY_BYTES];
  HQC_secret_key_from_string(x, y, pk, sk);
  
  // Compute v - u.y
  uint8_t tmp1 [HQC_VEC_N_SIZE_BYTES] = {0};
  vect_resize(tmp1, HQC_PARAM_N, v, HQC_PARAM_N1N2);
  uint8_t tmp2 [HQC_VEC_N_SIZE_BYTES] = {0};
  ...
  vect_add(tmp2, tmp1, tmp2, HQC_VEC_N_SIZE_BYTES);
    ...
    printf("\n\nv: "); vect_print(v, HQC_VEC_N1N2_SIZE_BYTES);
    printf("\n\ny: "); vect_print(y, HQC_VEC_N_SIZE_BYTES);
    printf("\n\nv - u.y: "); vect_print(tmp2, HQC_VEC_N_SIZE_BYTES);
  #endif
