...
void tensor_code_encode(uint8_t* em, uint8_t* m) {
  ...
  #ifdef VERBOSE
    printf("\n\nBCH code word: "); vect_print(tmp, HQC_VEC_N1_SIZE_BYTES);
    printf("\n\nTensor code word: "); vect_print(em, HQC_VEC_N1N2_SIZE_BYTES);
  #endif
}
...
void tensor_code_decode(uint8_t* m, uint8_t* em) {
  ...
  #ifdef VERBOSE
    printf("\n\nRepetition decoding result (the input for the BCH decoding algorithm): "); vect_print(tmp, HQC_VEC_N1_SIZE_BYTES);
  #endif
}
