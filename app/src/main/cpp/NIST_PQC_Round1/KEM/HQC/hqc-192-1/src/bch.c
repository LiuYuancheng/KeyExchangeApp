...
int16_t gf_mod(int16_t i) {
  return (i < HQC_PARAM_GF_MUL_ORDER) ? i : i - HQC_PARAM_GF_MUL_ORDER; 
}
 ...
 *
 * This function encode a message of HQC_PARAM_K bits to a code word of size HQC_PARAM_N1 bits. Following \cite lin1983error (Chapter 4 - Cyclic Codes), we perform a systematic encoding using a linear (HQC_PARAM_N1 - HQC_PARAM_K)-stage shift register with feedback
...
void bch_code_encode(uint8_t* em, uint8_t* m) {
  uint8_t tmp1[HQC_PARAM_K];
  uint8_t tmp2[HQC_PARAM_N1];
  uint8_t g [HQC_PARAM_G];
  memset(tmp2, 0, HQC_PARAM_N1);
...
void message_to_array(uint8_t* o, uint8_t* v) { 
  for (uint8_t i = 0 ; i < HQC_VEC_K_SIZE_BYTES ; ++i)  {
...
void get_generator_poly(uint8_t* g) {
  ...
  char GENERATOR_POLY [] = "C26D64FCDEF44ED7521EF849C65108B0CD57A6B37D746F784083CD0929AFB5F8BF56337F1E89B2D583FF6BFB627D42F412F81F49A018E6348A8F89B2DC374226";

  char* g_poly_string = GENERATOR_POLY, *pos = g_poly_string;
  uint8_t g_bytes_size = (HQC_PARAM_G / 8) + 1;
...
void lfsr_encoder(uint8_t* em, uint8_t* g, uint8_t* m) {
  ...
  for (int i = HQC_PARAM_K-1; i >= 0; --i) {
    gate_value = m[i] ^ em[HQC_PARAM_N1 - HQC_PARAM_K - 1];

    if (gate_value) {
      for (int j = HQC_PARAM_N1 - HQC_PARAM_K - 1; j > 0; --j)  {
        em[j] = em[j-1] ^ g[j];       
      }
  ...
  // Add the message 
  int index = 0;
  for (int i = HQC_PARAM_N1 - HQC_PARAM_K ;  i < HQC_PARAM_N1 ; ++i) {
    em[i] = m[index];
    index++;
  }
}
...
void array_to_codeword(uint8_t* v, uint8_t* c) {
  for (uint16_t i = 0 ; i < (HQC_VEC_N1_SIZE_BYTES - 1) ; ++i) {
    for (uint8_t j = 0 ; j < 8 ; ++j) {
      v[i] |= c[j + i * 8] << j;
    }
  }

  for (uint8_t j = 0 ; j < HQC_PARAM_N1 % 8 ; ++j) {
    v[HQC_VEC_N1_SIZE_BYTES - 1] |= ((uint8_t) c[j + 8 * (HQC_VEC_N1_SIZE_BYTES - 1)]) << j;
  }
}
 ...
 *    <ol>
 *    <li> The first step is the computation of 2 * HQC_PARAM_DELTA (with HQC_PARAM_DELTA being the error correcting capacity of the BCH code) syndromes.
...
void bch_code_decode(uint8_t* m, uint8_t* em) {
  // Generate Galois Field GF(2^10) using the primitive polynomial defined in HQC_PARAM_POLY
  // GF(2^10) is represented by the lookup tables (Log Anti-Log tables)
  gf_tables* tables = gf_tables_init();
  gf_generation(tables);      

  // Calculate the 2 * HQC_PARAM_DELTA syndromes
  ...
  sigma_poly* sigma = sigma_poly_init(2 * HQC_PARAM_DELTA);
  ...
  uint16_t error_pos [HQC_PARAM_DELTA];
  memset(error_pos, 0, HQC_PARAM_DELTA * 2);
  uint16_t size = 0;
  ...
  uint8_t e[HQC_VEC_N1_SIZE_BYTES] = {0};
  error_poly_gen(e, error_pos, size);

  #ifdef VERBOSE
    printf("\n\nThe error polynomial e(x) in binary representation: "); vect_print(e, HQC_VEC_N1_SIZE_BYTES);
  #endif

  // Add the error polynomial and the received polynomial 
  uint8_t tmp[HQC_VEC_N1_SIZE_BYTES] = {0};
  vect_add(tmp, e, em, HQC_VEC_N1_SIZE_BYTES);
...
gf_tables* gf_tables_init() {
  ...
  tables->size = HQC_PARAM_GF_MUL_ORDER + 1;
...
void gf_generation(gf_tables* gf_tables) {
  const uint16_t k  = 1 << HQC_PARAM_M; // k = 2^m = 2^10
  const uint16_t poly = HQC_PARAM_POLY; // get the primitive polynomial
  uint16_t val = 1;
  uint16_t alpha = 2; // alpha the root of the primitive polynomial is the primitive element of GF(2^10)

  for(int i = 0 ; i < HQC_PARAM_GF_MUL_ORDER ; ++i){
  ...
  gf_tables->antilog_tab[HQC_PARAM_GF_MUL_ORDER] = 1; 
...
syndrome_set* syndrome_init() {
  ...
  synd_set->size = 2 * HQC_PARAM_DELTA;
 ...
 * \brief Generates 2 * HQC_PARAM_DELTA syndromes from the received vector
 *
...
void syndrome_gen(syndrome_set* synd_set, gf_tables* tables, uint8_t* v) {
  uint8_t tmp_array[HQC_PARAM_N1];
  // For clarity of computation we separate the coordinates of the vector v by putting each coordinate in an unsigned char.
  for (uint8_t i = 0; i < (HQC_VEC_N1_SIZE_BYTES - 1) ; ++i) {
    for (uint8_t j = 0; j < 8; ++j) {
      tmp_array[j + i * 8] = (v[i] >> j) & 0X01;  
    }
  }

  for (uint8_t i = 0; i < HQC_PARAM_N1 % 8 ; ++i) {
    tmp_array [i + (HQC_VEC_N1_SIZE_BYTES - 1) * 8] = (v[HQC_VEC_N1_SIZE_BYTES -1] >> i) & 0x01;
  }

  // Evaluation of the polynomial corresponding to the vector v in alpha^i for i in {1, ..., 2 * HQC_PARAM_DELTA}
  for(uint16_t i = 0; i < HQC_PARAM_N1 ; ++i) {
    int tmp_value = 0;
    if(tmp_array[i]) {
...
void get_error_location_poly(sigma_poly* sigma, gf_tables* tables, syndrome_set* synd_set) {
  ...
  sigma_poly* sigma_rho = sigma_poly_init(2 * HQC_PARAM_DELTA);
  sigma_poly* sigma_copy = sigma_poly_init(2 * HQC_PARAM_DELTA);
  int k, pp = -1;
  // initializations
  sigma_rho->deg = 0;
  sigma_rho->value[0] = 1;
...
void chien_search(uint16_t* error_pos, uint16_t* size, gf_tables* tables, sigma_poly* sigma) {
  ...
  int k = HQC_PARAM_GF_MUL_ORDER - HQC_PARAM_N1;
  ...
  *size = 0;
  for(int i = k + 1 ; i <= HQC_PARAM_GF_MUL_ORDER ; ++i) {
    int sum = 0;
    int j = sigma->deg + 1;
    while(--j) {
      if (sigma->value[j] != -1) {
    ...
    if (sum == 1) {
      error_pos[*size] = HQC_PARAM_GF_MUL_ORDER - i;
      ++(*size);
    }
  }
}
 ...
 *  
 * This function recover the message (informations bits) from a code word. Since we have performed a systematic encoding, the message is the last HQC_PARAM_K bits of the vector v.
 *
...
void message_from_codeword(uint8_t* o, uint8_t* v) {
  int val = HQC_PARAM_N1 - HQC_PARAM_K;

  uint8_t mask_m1 = 0xC0;
  uint8_t mask_m2 = 0x3F;
...
cyclotomic_sets* cyclotomic_init() {
  ...
  c_tab->nb_c = 0;
  c_tab->tab = (int8_t*) calloc(HQC_PARAM_GF_MUL_ORDER + 1, sizeof(int8_t));
  return c_tab;
}
