 ...
 * To generate the vector we have to sample uniformly at random values in the interval [0, HQC_PARAM_N -1]. Suppose the HQC_PARAM_N is equal to \f$ 70853 \f$, to select a position \f$ r\f$ the function works as follow:
 *  1. It makes a call to the HQC2561_seedexpander function to obtain a random number \f$ x\f$ in \f$ [0, 2^{24}[ \f$.
...
void vect_fixed_weight(uint8_t* v, const uint16_t weight, AES_XOF_struct* ctx) {
  ...
  for(uint32_t i = 0; i < weight ; ++i) {
    ...
    random_data = random_data % HQC_PARAM_N;
 ...
 * \brief Generates a random vector of dimension <b>HQC_PARAM_N</b>
 *
 * This function generates a random binary vector of dimension <b>HQC_PARAM_N</b>. It generates a random 
 * array of bytes using the HQC2561_seedexpander function, and drop the extra bits using a mask.
 *
 * \param[in] v Pointer to an array
 * \param[in] ctx Pointer to the context of the seed expander
 */
void vect_set_random(uint8_t* v, AES_XOF_struct* ctx) {
  uint8_t rand_bytes[HQC_VEC_N_SIZE_BYTES] = {0};

  seedexpander(ctx, rand_bytes, HQC_VEC_N_SIZE_BYTES);

  memcpy(v, rand_bytes, HQC_VEC_N_SIZE_BYTES);
  v[HQC_VEC_N_SIZE_BYTES - 1] &= BITMASK(HQC_PARAM_N, 8);
}

/**
 * \fn void vect_set_random_from_HQC2561_randombytes(uint8_t* v)
 * \brief Generates a random vector
 *
 * This function generates a random binary vector. It uses the the HQC2561_randombytes function.
 *
 * \param[in] v Pointer to an array
 */
void vect_set_random_from_randombytes(uint8_t* v) {
  uint8_t rand_bytes [HQC_VEC_K_SIZE_BYTES] = {0};

  randombytes(rand_bytes, HQC_VEC_K_SIZE_BYTES);
  memcpy(v, rand_bytes, HQC_VEC_K_SIZE_BYTES);
} 
...
void vect_resize(uint8_t* o, uint32_t size_o, uint8_t* v, uint32_t size_v) {
  if(size_o < size_v) {
    memcpy(o, v, HQC_VEC_N1N2_SIZE_BYTES); 
    uint8_t mask = 0x7F;
    int val = 8 - (size_o % 8); 

    for(int i = 0; i < val; ++i) {
      o[HQC_VEC_N1N2_SIZE_BYTES - 1] &= (mask >> i);
    }
  } else { 
    memcpy(o, v, HQC_VEC_N_SIZE_BYTES);
  }
}
