...
void repetition_code_encode(uint8_t* em, uint8_t* m) {
  ...
  for(uint16_t i = 0; i < (HQC_VEC_N1_SIZE_BYTES - 1); ++i)  {
    for(uint8_t j = 0; j < 8; ++j) {
      val = (m[i]  >> j) & 0x01;
      if(val){
        uint32_t index = (i * 8 + j) * HQC_PARAM_N2; 
        for (uint8_t k = 0; k < HQC_PARAM_N2; ++k) {
          tmp[index + k] = 1;
        }
      }
    } 
  ...
  for(uint8_t j = 0; j < (HQC_PARAM_N1 % 8); ++j) {
    uint8_t val = (m[HQC_VEC_N1_SIZE_BYTES - 1] >> j) & 0x01;
    if(val){
      uint32_t index = ((HQC_VEC_N1_SIZE_BYTES - 1) * 8 + j) * HQC_PARAM_N2; 
      for(uint8_t k = 0; k < HQC_PARAM_N2; ++k) {
        tmp[index + k] = 1;
      }
    }
 ...
 *
 * We use a majority decoding. In fact we have that HQC_PARAM_N2 = 2 * HQC_PARAM_T + 1, thus,
 * if the Hamming weight of the vector is greater than HQC_PARAM_T, the code word is decoded
...
void repetition_code_decode(uint8_t* m, uint8_t* em) {
  ...
  int k = 1;
  int weight = 0;
  for(uint16_t i = 0; i < HQC_VEC_N1N2_SIZE_BYTES; ++i) {
    for(uint8_t j = 0; j < 8; ++j) {
      if((em[i] >> j) & 0x01) {
        weight ++;
      } 
      if(!(k % HQC_PARAM_N2)) {
        if(weight >= (HQC_PARAM_T + 1)) {
          int index = t / 8;
          m[index] |= 0x01 << (t % 8);
          weight = 0;
          t++;
...
void array_to_rep_codeword(uint8_t* o, uint8_t* v) {
  for(uint16_t i = 0 ; i < (HQC_VEC_N1N2_SIZE_BYTES - 1) ; ++i) {
    for(uint8_t j = 0 ; j < 8 ; ++j) {
      o[i] |= v[j + i * 8] << j;
    }
  }
  
  for(uint8_t j = 0 ; j < HQC_PARAM_N1N2 % 8 ; ++j) {
    o[HQC_VEC_N1N2_SIZE_BYTES - 1] |= ((uint8_t) v[j + 8 * (HQC_VEC_N1N2_SIZE_BYTES - 1)]) << j;
  }
}
